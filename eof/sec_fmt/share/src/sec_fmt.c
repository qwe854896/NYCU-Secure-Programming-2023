...
void response_regist(char *uuid)
{
	...
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "{"); // size: 1

	key = "\"uuid\"";
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", key); // size: 6
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", ": "); // size: 2
	val = "\"";
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", val); // size: 1
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", uuid); // size: 32
	val = "\"";
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", val); // size: 1
	key = "%s";
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), key, ","); // size: 1

	key = "\"status\"";
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", key); // size: 8
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", ": "); // size: 2
	val = "\"register success\"";
	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "%s", val); // size: 18
	key = "%s";

	__sprintf_chk(&buf[strlen(buf)], 1, sizeof(buf), "}"); // size: 1

	puts(buf); // Total size: 73
}
...
void response_regist_error(char *name, char *id)
{
	...
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[3]); // size: 1, "{"

	key = "\"name\"";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], key); // size: 2, "%s"
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], str[5]); // size: 2, ": "
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], str[7]); // size: 1, "\""
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], name); // size: 512, 0x200
	val = "\"";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], val); // size: 1, "\""
	key = "%s";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, key, str[6]); // size: 1, ","

	key = "\"id\"";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], key); // size: 2, "%s"
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], str[5]); // size: 2, ": "
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], str[7]); // size: 1, "\""
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], id); // size: 256, 0x100
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], str[7]); // size: 1, "\""
	key = "%s";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, key, str[6]); // size: 1, ","

	key = "\"msg\"";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], key); // size: 5, "%s"
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], str[5]); // size: 2, ": "
	val = "\"";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], val); // size: 1, "\""
	...
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], val); // size:  301
	val = "So sorry, ";
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[2], val); // size: 9
	__sprintf_chk(&buf[strlen(buf)], 1, 0x400, str[1], name); // size: 512, 0x200
	strcat(buf, ".");
	val = "\"";
