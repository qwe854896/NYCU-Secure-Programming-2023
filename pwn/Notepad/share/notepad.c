...
void command_login(int fd, char *username, char *password)
{
    ...
    struct Command cmd; // 164 bytes
    memset(&cmd, 0, sizeof(cmd));
    cmd.cmd = CMD_Login;                               // uint32_t 4 bytes
    strcpy(cmd.args, username);                        // cmd.args 128 bytes, username 0x20 bytes
    strcpy(&cmd.args[strlen(cmd.args) + 1], password); // cmd.args 128 bytes, password 0x20 bytes
    write(fd, &cmd, sizeof(cmd));
    struct Response res; // 260 bytes
    ...
    {
        Token = strdup(res.res); // 256 bytes
        puts("Login Success!");
    }
    else
    {
...
void command_register(int fd, char *username, char *password)
{
    ...
    struct Command cmd; // 164 bytes
    memset(&cmd, 0, sizeof(cmd));
    cmd.cmd = CMD_Register;                            // uint32_t 4 bytes
    strcpy(cmd.args, username);                        // cmd.args 128 bytes, username 0x20 bytes
    strcpy(&cmd.args[strlen(cmd.args) + 1], password); // cmd.args 128 bytes, password 0x20 bytes
    write(fd, &cmd, sizeof(cmd));
    struct Response res;
...
void command_newnote(int fd, char *notename, char *content)
{
    ...
    struct Response res; // 260 bytes
    memset(&res, 0, sizeof(res));
    cmd.cmd = CMD_NewNote;                         // uint32_t 4 bytes
    strcpy(cmd.token, Token);                      // cmd.token 32 bytes
    strncpy(cmd.args, notename, sizeof(cmd.args)); // cmd.args 128 bytes, notename 128 bytes
    write(fd, &cmd, sizeof(cmd));                  // 164 bytes
    if (read(fd, &res, sizeof(res)) != sizeof(res))
    {
        puts("Error while recv backend response.");
        return;
    ...
    int newfile_fd = open(res.res, O_RDWR); // res.res should be a file path
    if (newfile_fd < 0)
    {
        puts("Note create failed.");
        return;
    }
    write(newfile_fd, content, strlen(content)); // strlen(content) <= 1024
    close(newfile_fd);
    puts("Note created!");
}
...
int openfile(int fd, char *notename, off_t offset, int oflag)
{
    ...
    struct Response res; // 260 bytes
    memset(&res, 0, sizeof(res));
    cmd.cmd = CMD_GetFolder;  // uint32_t 4 bytes
    strcpy(cmd.token, Token); // cmd.token 32 bytes
    write(fd, &cmd, sizeof(cmd));
    if (read(fd, &res, sizeof(res)) != sizeof(res))
    {
    ...
    char path[128];
    // strcpy(path, res.res);
    snprintf(path, sizeof(path), "%s%s.txt", res.res, notename); // 256 bytes + 128 bytes
    // char rpath[128];
    // realpath(path, rpath);
    // puts(rpath);
...
void command_shownote(int fd, char *notename, off_t offset)
{
    ...
    char buf[128];
    ssize_t readlen = read(filefd, buf, sizeof(buf)); // 128 bytes
    if (readlen <= 0)
    {
...
int main()
{
    ...
    while (1)
    {
        ...
        if (backendfd <= 0)
            errorexit("Couldn't connect to backend.");
        switch (choice)
        {
        case 1:
            printf("Username: ");
